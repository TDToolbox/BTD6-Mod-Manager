...
namespace BTD6_Mod_Manager.Lib.Web
{
    public class UpdateHandler
    {
        ...
        public void HandleUpdates()
        {
            ...
            {
                Log.Output("BTD6 Mod Manager is up to date!");
                return;
            }

            Log.Output("An update is available for BTD6 Mod Manager!");
            AskInstallUpdates();
            DownloadUpdates();
            ExtractUpdater();

            Log.Output("Closing BTD6 Mod Manager to finish update", OutputType.Both);
            LaunchUpdater();
            Environment.Exit(0);
        }
        ...
        private void ExtractUpdater()
        {
            ...
            foreach (var file in files)
            {
                ...
                Log.Output($"Extracting file: {filename}");
                using (ZipArchive archive = ZipFile.OpenRead(file.FullName))
                {
                    foreach (ZipArchiveEntry entry in archive.Entries)
                    {
                        ...
                        if (destinationPath.StartsWith(Environment.CurrentDirectory, StringComparison.Ordinal))
                        {
                            ...
                            Log.Output($"Extracting file: {destinationPath}");
                        }
                    }
                }
            }
        ...
        private void LaunchUpdater()
        {
            ...
            {
                Log.Output("ERROR! Unable to find updater. You will need to close BTD6 Mod Manager" +
